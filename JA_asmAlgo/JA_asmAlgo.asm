...
;============ DECODING ============ 
decoding PROC
;uzywane rejestry: RSI(wskaznik na msg) |  | RBX(licznik inner_for) | R15(licznik outer_for) | R8(koniec wiadomosci mEnd)

	mov RSI, RDX	;zapisuje wskaznik na tablice z wiadomoscia, aby moc uzywac instrukcji mul, ktora potrzebuje rejestru RDX do poprawnego dzialania
	xor bl, bl		;licznik "i" zapisany w rejestrze al wewnetrznej petli for, ktora w cpp wyglada tak: for(int i=1; i<=24; i+=3)
	inc bl			;inkrementujê rejestr al, aby docelowo byl rowny 1

	xor r15d,r15d	;licznik zewnetrznej petli for. Petla w cpp wyglada tak: for(int i=0; i<mEnd; i++), gdzie mEnd to wartosc zaladowana do rejestru r15

;---------------- OUTER FOR ----------------		
outer_for:	
;---------------- INNER FOR ----------------
		
		mov r14b, [RSI + R15]	;czytam przy pomocy iteratora zewnetrznej petli (RBX) jeden znak (bajt) z wiadomosci.
			inner_for:				;petla wyglada w cpp tak: for(int i=1; i<=24; i+=3) poniewaz kodujemy co trzeci bajt pliku BMP -> 1 bajt wiadomosci = 24 bajty pliku bmp.
					...
					mov al, BMP_BYTE_COUNT_FOR_ONE_CHAR;w rejestrze eax znajdowac sie bedzie offset do miejsca w tablicy, do ktorego chce zaladowac porzadany bit(r13b).
...
;---------------- INNER FOR ----------------
	...
	xor bl, bl			;resetuje licznik wewnetrznej petli for (inner_for)
